...
public class GoogleSignInFragment extends Fragment implements
  ...
  public void onConnected(@Nullable Bundle connectionHint) {
    ...
    boolean hasConnectedAuth = mGoogleApiClient.hasConnectedApi(Auth.GOOGLE_SIGN_IN_API);
    if (hasConnectedAuth) {
      GoogleSignInHelper.logDebug("has connected auth!");
    }

    DoSignIn(!hasConnectedAuth);
  }
  ...
  public GoogleSignInAccount getAccount() {
    return request.getPendingResponse().await().getAccount();
  }
  ...
  public int getStatus() {
    ...
    return request.getPendingResponse().await().getStatus().getStatusCode();
  }
